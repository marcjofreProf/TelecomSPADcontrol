...
private:// Variables	
	...
	unsigned int pru1_cycles_period = 50; //512; // Initial number of clocks per cycle (it has to be power of 2). Sets the frequency of the geiger gating operation. Notice that the actual period is multiplied by PRUclockStepPeriodNanoseconds.
	unsigned int pru1_delay_first_off = 10; // 24; // Initial number of relative clocks/2 - 1 of the first off. At least 1.
	...
	// SPAD control
	double voltage_error=1.0; // Initialization
    ...
    bool InvDutyCycleOper=false; // false: regular definition of duty cycle; true: inverted operation with respect the duty cycle
